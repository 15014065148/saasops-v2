 ...
 */
	...
	public static String formatOnlineTime(BigInteger time) {
		...
		} else if (time.compareTo(new BigInteger("0")) == 1) {
			buf.append("少于1分");
		}
		return buf.toString();
	}
    ...
    public static int getTimesSession(HttpSession session,String username) {
        Integer times =(Integer)session.getAttribute(ApiConstants.LOGINTIMES_KEY+username);
        session.setAttribute(ApiConstants.LOGINTIMES_KEY+username, times == null ? 1 : times.intValue()+1);
        return times == null ? 1 : times.intValue()+1;
    }
