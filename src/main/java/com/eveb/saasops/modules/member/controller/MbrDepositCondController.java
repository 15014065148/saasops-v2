...
@Api(value = "MbrDepositCond", description = "会员组存款条件设置")
public class MbrDepositCondController {
	...
	@ApiImplicitParams({
			@ApiImplicitParam(name = "token", value = "token头部，随便填数字", required = true, dataType = "Integer", paramType = "header") })
	public R update(@RequestBody MbrDepositCond mbrDepositCond) {
	...
	private void verifyDepositCond(MbrDepositCond mbrDepositCond) {
		...
		if (mbrDepositCond.getFeeAvailable() == Available.enable) {
			Assert.isNumeric(mbrDepositCond.getFeeHours(), "时限只能为数字,并且最大长度为12位!", 12);
