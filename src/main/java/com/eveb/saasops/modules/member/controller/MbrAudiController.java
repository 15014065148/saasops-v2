...
@Api(value = "MbrAccountAudit", description = "会员稽核")
public class MbrAudiController extends AbstractController {
    ...
    @ApiImplicitParams({@ApiImplicitParam(name = "token", value = "token头部，随便填数字", required = true, dataType = "Integer", paramType = "header")})
    public R updateAudit(@RequestBody List<AuditDetailDto> detailDtos) {
        auditService.updateAccountAuditList(detailDtos, getUser().getUsername());
        return R.ok();
    }
    ...
    @ApiImplicitParams({@ApiImplicitParam(name = "token", value = "token头部，随便填数字", required = true, dataType = "Integer", paramType = "header")})
    public R clearAudit(@RequestParam("loginName") String loginName) {
        Assert.isBlank(loginName, "会员名不能为空");
        auditService.clearAccountAudit(loginName, getUser().getUsername(), CommonUtil.getSiteCode(), null);
        return R.ok();
    }
    ...
    @ApiOperation(value = "修改稽核只能更新不违规的", notes = "修改稽核只能更新不违规的")
    @ApiImplicitParams({@ApiImplicitParam(name = "token", value = "token头部，随便填数字", required = true, dataType = "Integer", paramType = "header")})
    public R updateNormal(@ModelAttribute MbrAuditBonus auditBonus) {
        auditService.updateNormal(auditBonus, getUser().getUsername());
        return R.ok();
    }
    ...
    @ApiImplicitParams({@ApiImplicitParam(name = "token", value = "token头部，随便填数字", required = true, dataType = "Integer", paramType = "header")})
    public R addAuditBonus(@ModelAttribute MbrAuditBonus auditBonus) {
        Assert.isBigDecimalNum(auditBonus.getAuditAmount(), "只能为正整数");
        auditService.addAuditBonus(auditBonus, getUser().getUsername());
        return R.ok();
    }
    ...
    @ApiImplicitParams({@ApiImplicitParam(name = "token", value = "token头部，随便填数字", required = true, dataType = "Integer", paramType = "header")})
    public R clearAuditBonus(@ModelAttribute MbrAuditBonus auditBonus) {
        Assert.isNull(auditBonus.getId(), "ID不能为空");
        auditService.clearAuditBonus(auditBonus, getUser().getUsername());
        return R.ok();
    }
    ...
    public R getPlatformMaxTime(@RequestParam("loginName") String loginName) {
        ...
        return R.ok().put(analysisService.getPlatformMaxTime(CommonUtil.getSiteCode(), null));
    }
