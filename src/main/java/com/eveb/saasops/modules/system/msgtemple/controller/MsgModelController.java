package com.eveb.saasops.modules.system.msgtemple.controller;
...
@Api(value = "MsgModel", description = "信息模板")
public class MsgModelController  extends AbstractController {
    ...
    @ApiImplicitParams({@ApiImplicitParam(name = "token", value = "token头部，随便填数字", required = true, dataType = "Integer", paramType = "header")})
    public R save(@RequestBody MsgModel msgModel) {
    	return msgModelService.save(msgModel,getUser().getUsername());
    }
    ...
    @PostMapping("/update")
    @ResponseBody
    @RequiresPermissions("msgtemple:msgtemple:update")
    @ApiOperation(value="修改", notes="修改")
    @ApiImplicitParams({@ApiImplicitParam(name = "token", value = "token头部，随便填数字", required = true, dataType = "Integer", paramType = "header")})
    public R update(@RequestBody MsgModel msgModel) {
            msgModelService.update(msgModel);
        return R.ok();
    }
    ...
    @ApiImplicitParams({@ApiImplicitParam(name = "token", value = "token头部，随便填数字", required = true, dataType = "Integer", paramType = "header")})
    public R delete(@RequestBody MsgModel msgModel) {
            msgModelService.deleteBatch(msgModel.getIds());
        return R.ok();
    }
