package com.eveb.saasops.modules.system.agencydomain.controller;
...
import com.eveb.saasops.modules.system.agencydomain.service.SystemAgencyUrlService;
...
@Api(value = "SystemAgencyUrl", description = "")
public class SystemAgencyUrlController {
    ...
    public R list(@ModelAttribute SystemAgencyUrl systemAgencyUrl, @RequestParam("pageNo") @NotNull Integer pageNo, @RequestParam("pageSize") @NotNull Integer pageSize,String orderBy) {
        return R.ok().put("page", systemAgencyUrlService.queryListPage(systemAgencyUrl,pageNo,pageSize,orderBy));
    }
    ...
    @ApiImplicitParams({@ApiImplicitParam(name = "token", value = "token头部，随便填数字", required = true, dataType = "Integer", paramType = "header")})
    public void ExportExcel(SystemAgencyUrl domain, HttpServletResponse response){
        systemAgencyUrlService.exportExcel(domain,response);
    }
}
