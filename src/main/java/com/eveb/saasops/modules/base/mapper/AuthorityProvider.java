...
public class AuthorityProvider extends MapperTemplate {
    ...
    public String selectInAuth(MappedStatement ms){
        ...
        if(!tableName.equals("mbr_group")){
            sql.append(SqlHelper.whereAllIfColumns(entityClass, isNotEmpty()));
        }else {
            sql.append(this.whereAllIfColumns(entityClass, isNotEmpty(), tableName(entityClass)));
        }
    ...
    private static String generateAuthSql(String tableName){
        ...
        return tableName.equals("mbr_group")?"<if test=\" baseAuth.groupIds !='' and  baseAuth.groupIds != null \"> AND ID "+groupAuth+"</if>":null;
    }
