...
import com.eveb.saasops.api.modules.user.service.IbcService;
...
public class IbcTransferService {
    ...
    public BillManBooleanDto ibcTransferIn(BillRequestDto requestDto, TGmApi gmApi) {
        BillManBooleanDto billManBooleanDto = new BillManBooleanDto();
        MbrDepotWallet mbrWallet = ibcService.createMember(gmApi, requestDto.getAccountId(), requestDto.getLoginName());
        ...
        Integer state = ibcService.deposit(gmApi, requestDto.getLoginName(), mbrBillManage.getAmount(), mbrBillManage.getOrderNo().toString());
        /**查询转账后余额*/
    ...
    public Boolean ibcTransferOut(BillRequestDto requestDto, TGmApi gmApi) {
        PtUserInfo info = ibcService.getBalance(gmApi, requestDto.getLoginName());
        if (Double.parseDouble(info.getBALANCE()) >= requestDto.getAmount().doubleValue()) {
            ...
            Integer state = ibcService.withdraw(gmApi, requestDto.getLoginName(), mbrBillManage.getAmount(), mbrBillManage.getOrderNo().toString());
            /**查询转账后余额*/
            mbrBillManage.setDepotAfterBalance(depotWalletService.queryDepotBalance(requestDto.getAccountId(), gmApi).getBalance());
            if (state == TransferStates.suc) {
